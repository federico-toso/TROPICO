...
phases(i).ne   = 1;             % number of elements per phase (for multi-shooting)
phases(i).nc   = 3;            % number of control nodes for each element within the phase
phases(i).x0 =          [100,  100, 85*pi/180, pi/2, -5*pi/180, 55*pi/180, 550e3];
phases(i).x0_no_opt =   [1, 1, 0, 0, 0, 0, 1];                              % id of fixed starting values
phases(i).xf =          [5e3, 1e3, 0, pi/2, 0, 0, 50e3];
...
phases(i).tof = [10; 60];          % bounds for time of flight for the phase
...
phases(i).ne   = 3;             % number of elements per phase (for multi-shooting)
phases(i).nc   = 6;            % number of control nodes for each element within the phase
% phases(i).x0 =          [15e3,  0.7*300, 0, pi/2, -5*pi/180, 55*pi/180, 550e3];
% phases(i).x0_no_opt =   [1, 1, 1, 0, 0, 0, 1];                              % id of fixed starting values
...
phases(i).fg = [0, 1, 0];     % starting guess for controls, can be any number of points > 2 for each control variable
phases(i).controls_no_match = [0, 1 ,0];
phases(i).tof = [60; 600];          % bounds for time of flight for the phase
